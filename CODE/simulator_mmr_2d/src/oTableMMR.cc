...
// Constructors
// ==========================================================================================================================================
oTableMMR::oTableMMR( int f_NbElem, int f_NbView, int f_NbSino, int f_MaxRingDiff, int f_Span, oScanner* fp_Scanner, int f_Verbose )
{
  // Verbose
  m_Verbose = f_Verbose;
  if (m_Verbose>=1) LogCout ("oTableMMR::Constructor() -> Initialize MMR conversion table" << endl);
  ...
  {
    LogCerr ("***** oTableMMR::Constructor() -> Number of elements read in sinogram is different from the normal number of the scanner !" << endl);
    exit(1);
  }
  if (mp_Scanner->GetNbView()%m_NbView!=0)
  {
    LogCerr ("***** oTableMMR::Constructor() -> Number of views read in sinogram is uncompatible with the scanner !" << endl);
    exit(1);
  }
  ...
  if (CalculateNbSino())
  {
    LogCerr ("***** oTableMMR::Constructor() -> Problem while calculating the number of sinograms !" << endl);
    exit(1);
  }
  ...
  {
    LogCerr ("***** oTableMMR::Constructor() -> Theoretical number of sinograms (" << m_NbCalculatedSino << ") differs from the given one (" << m_NbSino << ") !" << endl);
    exit(1);
  }
}
oTableMMR::oTableMMR( int f_Mash, int f_MaxRingDiff, int f_Span, oScanner* f_Scanner, int f_Verbose )
{
  // Verbose
  m_Verbose = f_Verbose;
  if (m_Verbose>=1) LogCout ("oTableMMR::Constructor() -> Initialize MMR conversion table" << endl);
...
oTableMMR::~oTableMMR()
{
  if (m_Verbose>=1) LogCout ("oTableMMR::Destructor() -> Destroy conversion table" << endl);
...
// ==========================================================================================================================================
int oTableMMR::ComputeSpanTables()
{
  if (m_Verbose>=1) LogCout ("oTableMMR::ComputeSpanTables() -> Compute the span conversion tables" << endl);

  // Get the number of rings
  int nb_rings = mp_Scanner->GetNbTotalAxialCrystals();

cout << "m_NbSino: " << m_NbSino << endl;
  ...
  if (!correct)
  {
    LogCerr ("***** oTableMMR::ComputeSpanTables() -> Inconsistence between the span (" << m_Span << ") and the maximum ring difference (" << m_MaxRingDiff << ") !" << endl);
    return 1;
  }
  ...
  for (int s=0; s<m_NbSegments; s++)
  {
    ...
    {
      // Check for sinogram plane index that may go overflow
      if (sino_bin>=m_NbSino) break;
      ...
      {
        if (m_Verbose>=4) LogCout ("        --> Sino index: " << sino_bin << " | Nb ring pairs: " << mp_NbRingPairsBySinoIndex[sino_bin] << " | From segment: " << segment_number << " | Rings sum: " << sum << endl);
        sino_bin++;
      }
    }
...
int oTableMMR::CalculateNbSino()
{
  ...
  if (!correct)
  {
    LogCerr ("***** oTableMMR::CalculateNbSino() -> Inconsistence between the span (" << m_Span << ") and the maximum ring difference (" << m_MaxRingDiff << ") !" << endl);
    return 1;
  }
  m_NbSegments = 2*m_NbAbsoluteSegments+1;
...
// ==========================================================================================================================================
int oTableMMR::ComputeRingTables()
{
  if (m_Verbose>=1) LogCout ("oTableMMR::ComputeRingTables() -> Compute the ring conversion tables" << endl);
...
//PATCH  int nb_total_trans_crystals = mp_Scanner->GetNbTotalTransCrystals();
...
// ==========================================================================================================================================
unsigned short int oTableMMR::GetNbRingPairsBySinoIndex(int f_Sino)
{
  return mp_NbRingPairsBySinoIndex[f_Sino];
}
void oTableMMR::GetRingPairBySinoIndex(int f_Sino, int f_RingPair, unsigned int* f_Ring1, unsigned int* f_Ring2)
{
  *f_Ring1 = mp_ListRingPairsBySinoIndex[f_Sino][f_RingPair][0];
  *f_Ring2 = mp_ListRingPairsBySinoIndex[f_Sino][f_RingPair][1];
}
void oTableMMR::GetRingPairsBySinoIndex(int f_Sino, unsigned int* f_Ring1, unsigned int* f_Ring2)
{
...
int oTableMMR::GetSinoIndexFromRingPair(unsigned int f_Ring1, unsigned int f_Ring2)
{
  return mp_RingPair2SinogramIndex[f_Ring1][f_Ring2];
}
void oTableMMR::GetCrystalIDsFromElemView(int f_Elem, int f_View, int* f_Crystal1, int* f_Crystal2)
{
  for (int m=0; m<m_Mash; m++)
  {
...
void oTableMMR::GetCrystalIDFromElemView(int f_Index, int f_Elem, int f_View, int* f_Crystal1, int* f_Crystal2)
{
  *f_Crystal1 = mp_ElemView2CrystalID1[f_Elem][f_View][f_Index];
  *f_Crystal2 = mp_ElemView2CrystalID2[f_Elem][f_View][f_Index];
}
void oTableMMR::GetElemViewFromCrystalIDs(int* f_Elem, int* f_View, int f_Crystal1, int f_Crystal2)
{
...
// ==========================================================================================================================================
void oTableMMR::ElemView2CrystalIDs(int f_Elem, int f_View, int* f_Crystal1, int* f_Crystal2)
{
  // Loop on mash
  for (int m=0; m<m_Mash; m++)
  {
...
// ==========================================================================================================================================
int oTableMMR::GetLORsFromSinoBin( int f_BinSino, int f_BinView, int f_BinElem, unsigned int* fp_ID1, unsigned int* fp_ID2 )
{
  // The number of LORs
  int nb_lors = 0;
