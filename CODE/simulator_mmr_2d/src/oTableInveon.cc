...
// Constructors
// ==========================================================================================================================================
oTableInveon::oTableInveon( int f_NbElem, int f_NbView, int f_NbSino, int f_MaxRingDiff, int f_Span, oScanner* fp_Scanner, int f_Verbose )
{
  // Verbose
  m_Verbose = f_Verbose;
  if (m_Verbose>=1) LogCout ("oTableInveon::Constructor() -> Initialize Inveon conversion table" << endl);
  ...
  {
    LogCerr ("***** oTableInveon::Constructor() -> Number of elements read in sinogram is different from the normal number of the scanner !" << endl);
    exit(1);
  }
  if (mp_Scanner->GetNbView()%m_NbView!=0)
  {
    LogCerr ("***** oTableInveon::Constructor() -> Number of views read in sinogram is uncompatible with the scanner !" << endl);
    exit(1);
  }
  ...
  if (CalculateNbSino())
  {
    LogCerr ("***** oTableInveon::Constructor() -> Problem while calculating the number of sinograms !" << endl);
    exit(1);
  }
  ...
  {
    LogCerr ("***** oTableInveon::Constructor() -> Theoretical number of sinograms (" << m_NbCalculatedSino << ") differs from the given one (" << m_NbSino << ") !" << endl);
    exit(1);
  }
}
oTableInveon::oTableInveon( int f_Mash, int f_MaxRingDiff, int f_Span, oScanner* f_Scanner, int f_Verbose )
{
  // Verbose
  m_Verbose = f_Verbose;
  if (m_Verbose>=1) LogCout ("oTableInveon::Constructor() -> Initialize Inveon conversion table" << endl);
...
// Destructor
// ==========================================================================================================================================
oTableInveon::~oTableInveon()
{
  if (m_Verbose>=1) LogCout ("oTableInveon::Destructor() -> Destroy conversion table" << endl);
...
// ==========================================================================================================================================
int oTableInveon::ComputeSpanTables()
{
  if (m_Verbose>=1) LogCout ("oTableInveon::ComputeSpanTables() -> Compute the span conversion tables" << endl);
  ...
  if (!correct)
  {
    LogCerr ("***** oTableInveon::ComputeSpanTables() -> Inconsistence between the span (" << m_Span << ") and the maximum ring difference (" << m_MaxRingDiff << ") !" << endl);
    return 1;
  }
...
// ==========================================================================================================================================
int oTableInveon::CalculateNbSino()
{
  // Get the number of rings
  int nb_rings = mp_Scanner->GetNbTotalAxialCrystals();
  ...
  if (!correct)
  {
    LogCerr ("***** oTableInveon::CalculateNbSino() -> Inconsistence between the span (" << m_Span << ") and the maximum ring difference (" << m_MaxRingDiff << ") !" << endl);
    return 1;
  }
  m_NbSegments = 2*m_NbAbsoluteSegments+1;
...
// ==========================================================================================================================================
int oTableInveon::ComputeRingTables()
{
  if (m_Verbose>=1) LogCout ("oTableInveon::ComputeRingTables() -> Compute the ring conversion tables" << endl);
...
// ==========================================================================================================================================
unsigned short int oTableInveon::GetNbRingPairsBySinoIndex(int f_Sino)
{
  return mp_NbRingPairsBySinoIndex[f_Sino];
}
void oTableInveon::GetRingPairBySinoIndex(int f_Sino, int f_RingPair, unsigned int* f_Ring1, unsigned int* f_Ring2)
{
  *f_Ring1 = mp_ListRingPairsBySinoIndex[f_Sino][f_RingPair][0];
  *f_Ring2 = mp_ListRingPairsBySinoIndex[f_Sino][f_RingPair][1];
}
void oTableInveon::GetRingPairsBySinoIndex(int f_Sino, unsigned int* f_Ring1, unsigned int* f_Ring2)
{
...
int oTableInveon::GetSinoIndexFromRingPair(unsigned int f_Ring1, unsigned int f_Ring2)
{
  return mp_RingPair2SinogramIndex[f_Ring1][f_Ring2];
}
void oTableInveon::GetCrystalIDsFromElemView(int f_Elem, int f_View, int* f_Crystal1, int* f_Crystal2)
{
  for (int m=0; m<m_Mash; m++)
  {
...
void oTableInveon::GetCrystalIDFromElemView(int f_Index, int f_Elem, int f_View, int* f_Crystal1, int* f_Crystal2)
{
  *f_Crystal1 = mp_ElemView2CrystalID1[f_Elem][f_View][f_Index];
  *f_Crystal2 = mp_ElemView2CrystalID2[f_Elem][f_View][f_Index];
}
void oTableInveon::GetElemViewFromCrystalIDs(int* f_Elem, int* f_View, int f_Crystal1, int f_Crystal2)
{
...
// ==========================================================================================================================================
void oTableInveon::ElemView2CrystalIDs(int f_Elem, int f_View, int* f_Crystal1, int* f_Crystal2)
{
  // Loop on mash
  for (int m=0; m<m_Mash; m++)
  {
...
// ==========================================================================================================================================
int oTableInveon::GetLORsFromSinoBin( int f_BinSino, int f_BinView, int f_BinElem, unsigned int* fp_ID1, unsigned int* fp_ID2 )
{
  // The number of LORs
  int nb_lors = 0;
